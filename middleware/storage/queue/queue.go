...
type Channel[T any] interface {
	...
	Subscribe(handler func(msg *Message[T]) error) error

	// Process 处理消息
	Process(handler func(msg *Message[T]) error) error

	// Next 获取下一条消息（阻塞）
	Next() (*Message[T], error)

	// TryNext 尝试获取下一条消息（非阻塞）
	TryNext() (*Message[T], error)
...
type Config struct {
	// StorageType 存储类型
	StorageType string `json:"storage_type,omitempty" yaml:"storage_type,omitempty" toml:"storage_type,omitempty" default:"memory"`
	// MaxRetries 最大重试次数
